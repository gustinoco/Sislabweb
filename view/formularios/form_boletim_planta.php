 ...
 *  
 */
$page_title = 'SisLabWeb 1.0 - Cadastrar boletim de Planta'; //Título da página
...
if (isset($editar)) {
    ...
    $plantas_boletim = $dao2->listar_planta_idboletim($editar);
    $plantas = $dao2->listar_basico_planta();
    ...
    $ultimoIdPlanta = $dao2->lista_ultimo_id();
    $pesquisadores = $daoPesquisador->lista_pesquisadores();
}
        ...
        for (i = 0; i < conflito.length; i++) {
            if (conflito[i] === idAmostra) {
                ...
                if (i === 0) {
                    ...
                    newdiv.innerHTML = "<tr><td><input style='width: 100px;' name='myInputs[" + contador + "][Id]' readonly='readonly' class='form-control' value='" + idAmostra + "' /></td>  <td>  <input class='form-control' style='width: 300px;' name='myInputs[" + contador + "][Identificacao]' class='form-control'/>       </td>   <td>     <input name='myInputs[" + contador + "][Macro]' type='checkbox' value='true'/>  </td>     <td>     <input name='myInputs[" + contador + "][Micro]' type='checkbox' value='true' />       </td>        <td>   <input name='myInputs[" + contador + "][Somente_n]'  type='checkbox' value='true' />                 </div></td>          <td>                           <button type='button' class='btn btn-xs btn-danger' onclick='RemoveTableRow(this)'>Remover</button>  </td></tr>";
...
//Caso editar seje cetado, header da pagina mostra Edição ou cadastar novo.
    ...
    <form onSubmit="return confere()" action="salvar_formulario_planta.php" method="post" role="form" class="form-inline">
        <div class="form">
        ...
        <?php if (isset($editar)) {
                ...
                <tbody>
                    <?php foreach ($plantas as $planta) { ?>     
                        ...
                        <tr>
                            <td>
                                <?php echo $planta->Id; ?>
                                <input name="Id_planta" type="hidden" value="<?php echo $planta->Id; ?>" />
                            </td>

                            <td>
                                <?php echo date("d/m/Y H:m:s", strtotime($planta->Data_cadastro)); ?>
                            </td>

                            <td>
                                <?php echo $planta->Pesquisador; ?>
                            </td>

                            <td>
                                <input onClick="addInput('dynamicInput', <?php echo $planta->Id; ?>)" class="btn btn-xs btn-success" value="Inserir" type="button">
            ...
            <table  class="table table-striped table-bordered" cellspacing="0" width="100%">
                ...
                <?php
                if (isset($editar)) {
                    foreach ($plantas_boletim as $planta) {
                        $contador_Amostras++;
                        ?><tr>
                            <td>
                                <input style='width: 100px;' name='myInputs[<?php echo $contador_Amostras ?>][Id]' readonly='readonly' class='form-control' value='<?php echo $planta->Id; ?>' />
                            </td> 

                            <td>  
                                <input class='form-control' style='width: 300px;' name='myInputs[<?php echo $contador_Amostras ?>][Identificacao]' class='form-control' value='<?php echo $planta->Identificacao; ?>'/>       
                            </td> 

                            <td> 
                                <input name='myInputs[<?php echo $contador_Amostras ?>][Macro]' type='checkbox' value='true'  <?php
                                if ($planta->Macro == 1) {
                                    echo checked;
                                }
                                ?> />
                            ...
                            <td>  
                                <input name='myInputs[<?php echo $contador_Amostras ?>][Somente_n]'  type='checkbox' value='true'   <?php
                                if ($planta->Somente_n == 1) {
                                    echo checked;
                                }
                                ?> />     
                            </td>     
                        ...
                        <div id="amostraPlanta">
                            <tbody id="dynamicInput" id="amostraPlanta">
                            </tbody>             
                        </div>
            ...
            <a class="btn btn-xs btn-info" style="font-size: 15px; margin-left: 80% " href="../plantas/digita_result_externos.php?id=<?php echo $editar; ?>">Editar Resultado das amostras</a>                              
        ...
        <?php } else {
            ...
            <center><div class="center">
                    ...
                    <label class='form-control'><?php echo $ultimoIdPlanta['MaximoId'] ?></label>

                    <p><label>Insira o intervalo</label></p>

                    De<input type="" class="form-control" name="Intervalo1" id="Intervalo1" value="<?php echo $ultimoIdPlanta['MaximoId'] + 1 ?>" />
                ...
                <BR><br>
                <label>Macro</label>
                <input name='Macro' type='checkbox' value='true' >
                <br>

                <label>Micro</label>
                <input name='Micro' type='checkbox' value='true' />
                <br>
                <label>Somente N</label>
                <input name='Somente_n' type='checkbox' value='true' />
...
<?php if (isset($editar)) {
    ?> <button class="btn btn-success" type="submit" value="Salvar alterações">Salvar alterações</button>
            <a href="../plantas/imprime_resultados_boletim.php?id=<?php echo $boletim->Id ?>" class="btn btn-info" role="button">Imprime resultados</a>

            <a href="plantas/digita_result_externos.php" class="btn btn-info" role="button">Exportar para Excel</a>
