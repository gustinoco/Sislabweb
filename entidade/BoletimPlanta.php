<?php
/**
 * Header do boletim de Planta
 * 
 * Todas funções e configurações do boletim de planta
 *
 * @package entidade
 * @subpackage BoletimPlanta
 * @since 1.0
 *  
 */
...
class BoletimPlanta {
     ...
     * @access public
     * @param BoletimPlanta $args Objeto com as relações do boletim
     */
     ...
     * @access public
     * @return BoletimPlanta Retona um array com todos os boletins cadastrados no BD.
     */
    function lista_boletim() {
        $conn = getConexao();
        $sql = "SELECT * FROM boletim_planta";
     ...
     * @access public
     * @return BoletimPlanta Retona apenas um boletim que está cadastrado no BD.
     */
    function lista_boletim_edicao($id) {
        $conn = getConexao();
        $sql = "SELECT * FROM boletim_planta WHERE Id =" . $id;
        $result = $conn->query($sql);
        while ($row = $result->fetch(PDO::FETCH_OBJ)) {
            $p = new BoletimPlanta();
            $p->mapear($row);
    ...
    function deletar($id_boletim) {
        try {
            if (!is_numeric($id_boletim))
                return;
            $conn = getConexao();
            $sql = 'DELETE FROM boletim_planta WHERE Id=:boletim_id';
    ...
    function lista_ultimo_id() {
        $conn = getConexao();
        $sql = "SELECT MAX(Id) AS MaximoId FROM boletim_planta";
     ...
     * @param BoletimPlanta $boletim Objeto boletim
     * @access public
     */
    function editar($boletim) {
        $sql = 'UPDATE boletim_planta
				SET 
    ...
    function salvar($boletim) {
        try {
            $conn = getConexao();
            $sql = "INSERT INTO boletim_planta (Pesquisa, Data_entrada, Id_cliente, Valor, Cultura, Sistema, Observacao, Pesquisador, Propriedade)
...
//retorna id da inserção
     ...
     * @access public
     * @return BoletimPlanta Retona um array de boletins de acordo com o cliente
     */
    function verifica_boletim($id_cliente) {
        try {
            $conn = getConexao();
            $sql = "SELECT * FROM boletim_planta WHERE Id_cliente = " . $id_cliente;
            $result = $conn->query($sql);
            while ($row = $result->fetch(PDO::FETCH_OBJ)) {
                $p = new BoletimPlanta();
                $p->mapear($row);
     ...
     * @access public
     * @return BoletimPlanta Retona um array de boletins de acordo com o pesquisador
     */
    function lista_boletim_pesquisador($login_pesquisador) {
        try {
            $conn = getConexao();
            $sql = "SELECT * FROM boletim_planta WHERE Pesquisador = '$login_pesquisador'";
            $result = $conn->query($sql);
            while ($row = $result->fetch(PDO::FETCH_OBJ)) {
                $p = new BoletimPlanta();
                $p->mapear($row);
